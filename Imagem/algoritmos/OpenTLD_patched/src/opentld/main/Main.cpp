...
void Main::doWork()
{
    ...
    cvtColor(cvarrToMat((const CvArr*) img), grey, CV_BGR2GRAY);
    ...
    while(imAcqHasMoreFrames(imAcq))
    {
        ...
        if(showOutput || saveDir != NULL)
        {
            ...
            CvRect box;
            box.x = tld->currBB->tl().x;
            box.y = tld->currBB->tl().y;
            box.width = tld->currBB->br().x - tld->currBB->tl().x;
            box.height = tld->currBB->br().y - tld->currBB->tl().y;
            switch (getBBFromUser(img, box, gui))
            {
                case PROGRAM_EXIT:
                goto exit;
