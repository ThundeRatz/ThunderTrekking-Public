...
void final_position(gps_coord_t *position, double dist, double bearing) {
	...
	position->latitude = asin(sin(latitude_inicial) * cos(dist_angular) +
		cos(latitude_inicial) * sin(dist_angular) * cos(bearing));
	position->longitude += atan2(sin(bearing) * sin(dist_angular) * cos(latitude_inicial),
		cos(dist_angular) - sin(latitude_inicial) * sin(position->latitude));
}
