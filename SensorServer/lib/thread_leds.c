#define LED_R    XX   // Mudar pras portas dos LEDs LED_R
#define LED_G    XX   // LED_G
#define LED_B    XX   // LED_B
...
#include <time.h>
#include "gpio_dma.h"
#include "udp_receiver.h"  // Mudar pra receiver
#include "ports.h"         // Colocar o UDP_LED 

#define MS	1000000



void __attribute__((noreturn)) *leds_thread(__attribute__((unused)) void *ignored) { //leds_thread (?)
	const struct timespec sleep_time = {.tv_sec = 0, .tv_nsec = 50 * MS};
    int8_t status_antigo = 0, status = 0;
    int socket = udp_receiver_init(UDP_LED); // Ver se tem que mudar pra receiver ou apagar ou outra coisa | Mudar pra UDP_LED(?)
    ...
    gpio_dma_output(LED_R);  //Mudar pra output 
    gpio_dma_output(LED_G);  //gpio_dma_output(LED_<RGB>)
