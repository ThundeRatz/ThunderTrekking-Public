...
int main(int argc, char** argv) {
	...
	Mat descriptors_cone;

#if DETECTOR == _SURF
	SURF detect_and_compute(400, 4, 2, true, true);
#elif DETECTOR == _ORB
	ORB detect_and_compute();
#elif DETECTOR == _BRISK
	BRISK detect_and_compute();
#elif DETECTOR == _FREAK
	FREAK detect_and_compute();
#endif
	//detect_and_compute.detect(cone_img, keypoints_cone);
	//detect_and_compute.compute(cone_img, keypoints_cone, descriptors_cone);
	detect_and_compute(cone_img, Mat(), keypoints_cone, descriptors_cone);
	
	/*
	SurfFeatureDetector detector(400, 4, 2, true, true);
	detector.detect(cone_img, keypoints_cone);
	SurfDescriptorExtractor extractor;
	extractor.compute(cone_img, keypoints_cone, descriptors_cone);
	*/
	...
	for (;;) {
		...
		if (good_matches.size() >= 4) {
			...
			double prop = ((double) scene_corners[2].x - (double) scene_corners[0].x) / (scene_corners[2].y - scene_corners[0].y) - cone_img.cols / cone_img.rows;
			if (prop > 0.3 && prop < 1 / 0.3) {
				...
				int center = (scene_corners[0].x + scene_corners[1].x) / 2 - frame.cols / 2;
